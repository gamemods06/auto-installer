...
EOF
	...
	cat << EOF >  /var/www/html/"$client"/"$client".conf
[Interface]
...
EOF
	...
	echo
	qrencode -t UTF8 < /var/www/html/"$client"/"$client.conf"
	...
	echo
	echo "The client configuration is available in:" /var/www/html/"$client.conf"
	...
	clear
			...
			echo
			echo "$client added. Configuration available in:" /var/www/html/"$client"/"$client.conf"
			exit
